...
function injectCanvasInteractingStyle(): void {
  ...
  style.textContent = `
    /* Click-shield for BROWSER panels on unfocused nodes. A webview's guest
       content is a separately-composited native layer the DOM dim overlay
       cannot reliably sit above, so without this the first click on an
       unfocused browser lands IN the page (navigating it) instead of focusing
       the node. pointer-events is embedder-side hit-testing and is honored.
