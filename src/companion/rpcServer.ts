...
// each to the hosted Companion capability object, and writes a `res` frame back.
...
export interface RpcServerOptions {
  ...
  hello?: Partial<Pick<HelloFrame, 'node' | 'os' | 'companionVersion' | 'protocolVersion'>>
}
...
export class RpcServer {
  ...
  constructor(
    private readonly api: Companion,
  ...
  start(): void {
    const hello: HelloFrame = {
      t: 'hello',
      companionVersion: this.opts.hello?.companionVersion ?? COMPANION_VERSION,
      protocolVersion: this.opts.hello?.protocolVersion ?? COMPANION_PROTOCOL_VERSION,
