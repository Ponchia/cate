// =============================================================================
// Bundles the cate-companion daemon into a single runtime-agnostic .cjs that
// can be shipped to a server / WSL and run by a plain Node (NOT Electron). Run
// via `npm run build:companion`.
//
...
export function syncCompanionVersion() {
  const pkg = JSON.parse(readFileSync(path.join(repoRoot, 'package.json'), 'utf-8'))
  const versionFile = path.join(repoRoot, 'src/companion/version.ts')
  const header =
    '// =============================================================================\n' +
    '// Companion version — GENERATED from package.json by `npm run build:companion`.\n' +
    ...
    '// =============================================================================\n\n'
  const body = `export const COMPANION_VERSION = '${pkg.version}'\n`
  const next = header + body
  if (readFileSync(versionFile, 'utf-8') !== next) {
    writeFileSync(versionFile, next)
    console.log(`[build:companion] version.ts -> ${pkg.version}`)
  }
  return pkg.version
}

export const companionBuildOptions = {
  entryPoints: [path.join(repoRoot, 'src/companion/index.ts')],
  bundle: true,
  platform: 'node',
  format: 'cjs',
  target: 'node20',
  outfile: path.join(repoRoot, 'dist-companion/companion.cjs'),
