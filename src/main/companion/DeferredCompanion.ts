// =============================================================================
// DeferredCompanion — a Companion proxy registered SYNCHRONOUSLY while the real
// companion (the LOCAL daemon) connects in the background. It lets the window
...
// real RemoteCompanion once the daemon is online.
//
// Shape rules (mirror RemoteCompanion's client-side behavior):
...
//     clear failure, not a confusing "No companion registered".
...
//     RemoteCompanion does for its own async start.
// =============================================================================
...
export class DeferredCompanion implements Companion {
  ...
  constructor(
    ...
    private readonly ready: Promise<Companion>,
  ) {
    const ready_ = this.ready
    // Await ready, then call the same method on the real companion.
    const d = <T>(fn: (c: Companion) => Promise<T>): Promise<T> => ready_.then(fn)
    ...
    this.file = {
      ...
      // once ready unless cancelled. Mirrors RemoteCompanion.file.searchContent.
      ...
      // unless unsubscribed. Mirrors RemoteCompanion.file.watch.
  ...
  // SYNC pass-throughs — identical to RemoteCompanion: the authoritative check
