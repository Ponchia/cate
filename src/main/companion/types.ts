...
export interface PtyCreateOptions {
  cols: number
  rows: number
  /** Companion-absolute, already validated working directory. */
  cwd: string
...
export interface AgentStartOptions {
  ...
  id: string
  /** Companion-absolute working directory (the daemon validates it). */
  cwd: string
...
// an ALREADY-VALIDATED, companion-absolute path. The IPC handler validates the
// raw locator path via `Companion.validate*` first, then passes the safe path
// here. This keeps these leaf ops pure fs — directly reusable by the standalone
// companion daemon in Phase 3. `importEntries` is the lone exception: it computes
...
// companion-path `cwd` and validate it internally (via validateCwd), mirroring
