...
const CLAUDE_EVENTS = ['SessionStart', 'UserPromptSubmit', 'Notification', 'Stop', 'SessionEnd']
...
// four bus events Cate tracks; the bus is otherwise chatty (message parts).
// ---------------------------------------------------------------------------

const OPENCODE_PLUGIN_SOURCE = `// Generated by the Cate runtime daemon (agent hook injection). Do not edit.
const ENDPOINT = process.env.${CATE_HOOK_ENDPOINT_ENV}
const TOKEN = process.env.${CATE_HOOK_TOKEN_ENV}
const TRACKED = new Set(["session.created", "session.status", "session.idle", "permission.asked"])
